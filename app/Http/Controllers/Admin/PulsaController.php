...
class PulsaController extends Controller
{
    public function index()
    {
        ...
        $pageTitle = 'Pulsa';
        $page = 'pulsa';
        $sum_monswallet = $this->info_sum_monswallet(2,1,2); 
        $sum_monswallet_failed = $this->info_sum_monswallet(2,2,2); 
        $sum_lifepoint = $this->info_sum_monswallet(2,1,3); 
        $sum_lifepoint_failed =$this->info_sum_monswallet(2,2,3); 
    
        $monswallet = $this->info_count_monswallet(2,1,2); 
        $monswallet_failed =$this->info_count_monswallet(2,2,2); 
        $lifepoint = $this->info_count_monswallet(2,1,3); 
        $lifepoint_failed = $this->info_count_monswallet(2,2,3); 
    ...
    public function data(Request $request)
    {
        ...
        if($request->from_date != '' || $request->to_date != ''  ){
            $data = PpobTransaction::join('users', 'users.id', '=', 'ppob_transactions.user_id')
                ...
                ->where('ppob_transactions.type_id','=',2) // 1=data 2=pulsa
    ...
    public function pricelist(){
        ...
        $responseApi = Curl::to(env("MOBILE_PULSA_URI")."/pulsa")
                ->withData( $header )
                ->asJson()
                ->post();
        return  $responseApi;
